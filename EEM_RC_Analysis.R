...
# vs standard of care pathway.   The analysis is conducted from the US payer 
# perspective. A state-transition (Markov)  model is being developed in R, 
# with an automated reporting via R-Markdown.  The   model R-code is currently 
# On completion of the development, the code will be separated into R-scripts 
# to be run separately from this report, with the results passed 
# as parameters to the report
...
# ++++++ T he structure of this R-script +++++++++++++++++
# Load libraries 
...
# The table below is for the convenience of model development and debugging
# to be deleted from the final version, if useful can be kept in 
# a supplementary attachment.
# Important, any new parameter to be included in the *.csv file 
# should be added after the last row in the params.csv file 
# to preserve the position of the existing inputs
# the convenience of the csv is to provide flexibility of defining model 
# parameters in one place and to configure the parameters 
# of deterministic and prob abilistic sensitivity analyses 
